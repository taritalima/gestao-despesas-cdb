...
class UsuarioMapperTest {
    ...
    void deveRetornarNullQuandoRequestForNull() {
        Usuario result = mapper.toDomain((UsuarioRequest) null);
        assertThat(result).isNull();
    }

    @Test
    void deveRetornarNullQuandoEntityForNull() {
        Usuario result = mapper.toDomain((UsuarioEntity) null);
        assertThat(result).isNull();
    }

    @Test
    void deveRetornarNullQuandoDomainForNull() {
        UsuarioEntity result = mapper.toEntity(null);
        assertThat(result).isNull();
    }

    @Test
    void deveRetornarNullQuandoResponseForNull() {
        UsuarioResponse result = mapper.toResponse(null);
        assertThat(result).isNull();
    }
}
